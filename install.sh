...
if [ "$FAIL" = 0 ]; then
  ...
  echo "  4. Save & Apply — PassWall2 перезапустится сам"
  echo "  5. Zapret:"
  echo "     sh <(wget -O - https://raw.githubusercontent.com/StressOzz/Zapret-Manager/main/Zapret-Manager.sh)"
  echo "  6. Сразу после Zapret исключить домены VPN, иначе туннель повиснет:"
  echo "     printf 'doxfie.top\ndoxfie.net\n' >> $ZEX && /etc/init.d/zapret restart"
  echo "===================================================================="
else
