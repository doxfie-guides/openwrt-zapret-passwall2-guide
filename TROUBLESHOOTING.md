...
> [!TIP]
> Если Save & Apply не перезапускает PassWall2 — виноват `ucitrack`: триггеры он регистрирует только при своём старте, а на момент загрузки роутера PassWall2 ещё не было. Лечится один раз: `/etc/init.d/ucitrack restart`. После этого Save & Apply работает сам. Проверка: `ubus call service list '{"name":"ucitrack","verbose":true}' | grep -c passwall2` — должно быть не ноль.
...
| Включить анализ данных GeoIP | ✅ если в правилах есть `geoip:` (в `common` есть `geoip:telegram`); снимать, только когда таких правил нет |
...
| параметр | значение |
|---|---|
| Метод TCP-проксирования | `TPROXY` (изначально `REDIRECT`) |
| **IPv6 TProxy** | ✅ — без неё проксируемые сайты по IPv6 идут мимо VPN с домашнего адреса |
...
|---|---|
| Прямые сайты открываются, а всё проксируемое висит; в клиенте на ПК живы только Hysteria и российские входы | стратегия Zapret `fake` бьёт по Reality-хендшейку до VPN-серверов. Дописать `doxfie.top` и `doxfie.net` в `/opt/zapret/ipset/zapret-hosts-user-exclude.txt`, затем `/etc/init.d/zapret restart`. Хватает доменов: nfqws видит SNI. Голые IP в этом файле не работают — исключения только резолвятся из доменов |
| «Мой налог» не открывается, остальное работает | не Zapret, а DNS: зону `gi.nalog.ru` NS ФНС не отдают резолверам Cloudflare, ответ SERVFAIL. Скрипт ставит под неё Google DoH на `127.0.0.1:5055`. Проверка: `nslookup -port=5055 lknpd.nalog.ru 127.0.0.1`. Если инстанс не слушает — смотреть `bootstrap_dns`: с двумя адресами он падает |
| `Core: Служба остановлена` сразу после установки | снята галка **«Включить модуль»** на вкладке «Общие параметры → Основной». Это первое, что надо проверить: узел и остальные галки к запуску отношения не имеют |
| Настройки сохранены, но `Core: Служба остановлена`, в журнале пусто | служба не перезапускалась. Разовое лечение причины: `/etc/init.d/ucitrack restart` — дальше Save & Apply справляется сам. Разово перезапустить: **System → Startup → passwall2 → Restart** (именно `passwall2`, а не `passwall2_server` — тот отвечает за режим VPN-сервера) |
...
| Проксируемый сайт видит домашний адрес или отвечает 403/451, хотя в правиле он есть | IPv6 идёт мимо туннеля: не стоит **IPv6 TProxy** (Дополнительные настройки, метод TCP — TPROXY). Одной FakeDNS не хватает: dnsmasq держит в кэше настоящие AAAA — после переноса домена в проксируемые и после рестартов, а `geoip:`-правила по IPv6 без TProxy не ловятся совсем. Проверка с ПК: `curl -s --resolve 'claude.ai:443:[2607:6bc0::10]' https://claude.ai/cdn-cgi/trace` — `loc` должен быть страной ноды, не `RU`. Если на ноде нет IPv6, сайты по-прежнему откроются: экзит сам резолвит домен из TLS. Не пройдёт только трафик без домена — например, Telegram по голым IPv6-адресам |
