...
**3. Привязка.** **Общие параметры → Правила разделения трафика** → напротив правил выбрать ноду. Затем перезапустить PassWall2.

> [!IMPORTANT]
> **После любого изменения настроек PassWall2 нужно перезапустить вручную.** Save & Apply сохраняет конфиг, но службу не трогает: procd-триггера у неё нет, а `ucitrack` в OpenWrt 25.12 больше не используется. Пока не перезапустите — работает старая конфигурация.
>
> Из веб-интерфейса: **System → Startup** → в списке найти `passwall2` → **Restart**. По SSH то же самое: `/etc/init.d/passwall2 restart`
