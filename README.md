...
**1. Подписка.** LuCI → **Services → PassWall 2 → Подписки → Добавить**. Поле **User-Agent** оставить `v2rayN`, иначе панель отдаст JSON вместо списка ссылок. Внизу поставить галку **«Отправлять HWID»** — без неё роутер не учитывается как устройство в подписке. Save & Apply, затем **обязательно** «Ручное обновление подписки» — само не подтянется.
